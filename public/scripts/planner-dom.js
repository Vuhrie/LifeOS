const $ = (id) => document.querySelector(id);

export const getPlannerUi = () => ({
  status: $("#planner-status"),
  lock: $("#planner-lock"),
  connect: $("#connect-google"),
  connectDrawer: $("#connect-google-drawer"),
  commit: $("#commit-draft"),
  generate: $("#generate-draft"),
  clear: $("#clear-draft"),
  stepPills: [...document.querySelectorAll(".step-pill")],
  stepPanels: [...document.querySelectorAll(".wizard-step")],
  prev: $("#step-prev"),
  next: $("#step-next"),
  wake: $("#wake-time"),
  sleep: $("#sleep-time"),
  horizonDays: $("#horizon-days"),
  lockedHours: $("#locked-hours"),
  needBreakfast: $("#need-breakfast"),
  needDinner: $("#need-dinner"),
  needMorningShower: $("#need-morning-shower"),
  needNightShower: $("#need-night-shower"),
  commitmentType: $("#commitment-type"),
  commitmentTitle: $("#commitment-title"),
  commitmentStart: $("#commitment-start"),
  commitmentEnd: $("#commitment-end"),
  commitmentDayWrap: $("#commitment-day-wrap"),
  commitmentDay: $("#commitment-day"),
  commitmentDateRangeRow: $("#commitment-date-range-row"),
  commitmentStartDate: $("#commitment-start-date"),
  commitmentEndDate: $("#commitment-end-date"),
  commitmentWeekdays: $("#commitment-weekdays"),
  commitmentWeekdayHint: $("#commitment-weekday-hint"),
  commitmentDays: $("#commitment-days"),
  addCommitment: $("#add-commitment"),
  commitmentsList: $("#commitments-list"),
  goalTitle: $("#goal-title"),
  goalDeadline: $("#goal-deadline"),
  goalPriority: $("#goal-priority"),
  goalHours: $("#goal-weekly-hours"),
  addGoal: $("#add-goal"),
  goalsList: $("#goals-list"),
  approveMajorGoalProposals: $("#approve-major-goal-proposals"),
  rejectMajorGoalProposals: $("#reject-major-goal-proposals"),
  majorGoalProposalsList: $("#major-goal-proposals-list"),
  minorGoalsList: $("#minor-goals-list"),
  tasksList: $("#tasks-list"),
  habitName: $("#habit-name"),
  habitFrequency: $("#habit-frequency"),
  habitDuration: $("#habit-duration"),
  habitWindow: $("#habit-window"),
  addHabit: $("#add-habit"),
  habitsList: $("#habits-list"),
  draftSchedule: $("#draft-schedule"),
  unscheduledList: $("#unscheduled-list"),
  warningsList: $("#warnings-list"),
  commitProgress: $("#commit-progress"),
  commitProgressPhase: $("#commit-progress-phase"),
  commitProgressBar: $("#commit-progress-bar"),
  commitProgressMeta: $("#commit-progress-meta"),
  commitProgressCurrent: $("#commit-progress-current"),
  commitProgressCounts: $("#commit-progress-counts"),
  commitProgressLog: $("#commit-progress-log"),
  aiBuildPrompt: $("#ai-build-prompt"),
  aiCopyPrompt: $("#ai-copy-prompt"),
  aiPromptOutput: $("#ai-prompt-output"),
  aiImportInput: $("#ai-import-input"),
  aiValidateImport: $("#ai-validate-import"),
  aiApplyImport: $("#ai-apply-import"),
  aiClearImport: $("#ai-clear-import"),
  aiStatus: $("#ai-assist-status"),
  aiChangedSince: $("#ai-changed-since"),
  aiTaskProgressNotes: $("#ai-task-progress-notes"),
  aiBriefNotes: $("#ai-brief-notes"),
  aiPriorityMajorGoal: $("#ai-priority-major-goal"),
});
